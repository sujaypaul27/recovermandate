...
public interface PaymentEventRepository extends JpaRepository<PaymentEvent, Long> {
    ...
    @org.springframework.data.jpa.repository.Query(
        ...
        "WHERE LOWER(p.razorpayPaymentId) LIKE LOWER(CONCAT('%', :query, '%')) " +
        "OR LOWER(p.eventType) LIKE LOWER(CONCAT('%', :query, '%')) " +
        "OR (s IS NOT NULL AND LOWER(s.razorpaySubscriptionId) LIKE LOWER(CONCAT('%', :query, '%'))) " +
        "OR (c IS NOT NULL AND LOWER(c.email) LIKE LOWER(CONCAT('%', :query, '%'))) " +
